...
static void anc_write_file_start(u8 *cmd, u32 id, u32 data_len)
{
    ...
    case FILE_ID_SPKEQ:
        ...
        __this->file_hdl = (u8 *)malloc(__this->file_len);
        __this->data_ptr = (u8 *)malloc(32);
        if ((__this->file_hdl == NULL) || (__this->data_ptr == NULL)) {
            cmd[0] = CMD_ANC_MODULE;
            cmd[1] = CMD_ANC_FAIL;
            chargestore_api_write(cmd, 2);
            log_info("anc eq file malloc err, %d\n", __this->file_len);
            if (__this->file_hdl) {
                free(__this->file_hdl);
                __this->file_hdl = NULL;
            }
        ...
        log_info("write spkeq start! : file_len : %d\n", __this->file_len);
        break;
#endif
    }
}
...
static void anc_write_file_data(u8 *cmd, u32 offset, u8 *data, u32 data_len)
{
    ...
    case FILE_ID_SPKEQ:
        ...
        if (__this->file_id == FILE_ID_SPKEQ) {
            log_info("spkeq data, offset=%d, data_len:%d\n", offset, data_len);
...
static void anc_write_file_end(u8 *cmd)
{
    ...
    case FILE_ID_SPKEQ:
        ...
        if (__this->file_hdl == NULL) {
            ...
            printf_buf(__this->file_hdl, __this->file_len);
            ret = spk_eq_spp_rx_packet(__this->file_hdl, __this->file_len);
            free(__this->file_hdl);
            free(__this->data_ptr);
            __this->file_hdl = NULL;
            __this->data_ptr = NULL;
