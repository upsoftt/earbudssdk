...
static void bt_tws_tone_play(unsigned char type)
{
    STATUS *p_tone = get_tone_config();
    if(type == 0) {
        tone_play_index(IDEX_TONE_BT_MODE, 1);
    } else if(type == SYNC_TONE_TWS_CONNECTED) {
        tone_play_index(p_tone->tws_connect_ok, 1);
    } else if(type == SYNC_TONE_PHONE_CONNECTED) {
...
int bt_tws_connction_status_event_handler(struct bt_event *evt)
{
    ...
    case TWS_EVENT_SYNC_FUN_CMD:
        ...
        } else if(reason == SYNC_CMD_APP_EQ1){
            printf("SYNC_CMD_APP_EQ1: %d\n");
			eq_file_set_by_index(0);
		} else if(reason == SYNC_CMD_APP_EQ2){
            printf("SYNC_CMD_APP_EQ2: %d\n");
			eq_file_set_by_index(1);
		} else if(reason == SYNC_CMD_APP_EQ3){
            printf("SYNC_CMD_APP_EQ3: %d\n");
			eq_file_set_by_index(2);
		} else if(reason == SYNC_CMD_APP_EQ4){
            printf("SYNC_CMD_APP_EQ4: %d\n");
			eq_file_set_by_index(3);
		} else if(reason == SYNC_CMD_APP_EQ5){
            printf("SYNC_CMD_APP_EQ5: %d\n");
			eq_file_set_by_index(4);
		} else if(reason == SYNC_CMD_APP_EQ6){
            printf("SYNC_CMD_APP_EQ6: %d\n");
			eq_file_set_by_index(5);
		} else if(reason == SYNC_CMD_FIND_ALL_TONE_PLAY){
            printf("SYNC_CMD_FIND_ALL_TONE_PLAY\n");
			tone_play(TONE_FIND_TONE, 1);
		} else if(reason == SYNC_CMD_FIND_ALL_TONE_SOTP){
            printf("SYNC_CMD_FIND_ALL_TONE_SOTP\n");
			tone_stop(0);
		} else if(reason == SYNC_CMD_AUTO_ROLE_OFF){
            printf("SYNC_CMD_AUTO_ROLE_OFF\n");printf("tws_api_auto_role_switch_disable\n");
			tws_api_auto_role_switch_disable();
		} else if(reason == SYNC_CMD_EQ_TONE){
            printf("SYNC_CMD_EQ_TONE\n");
			tone_play_no_tws(TONE_EQ_TONE, 1);
		} else if(reason == SYNC_CMD_FIND_L_TONE){
            printf("SYNC_CMD_FIND_L_TONE\n");
			if (get_bt_tws_connect_status()){
				if(bt_tws_get_local_channel() == 'L'){
					tone_play_no_tws(TONE_FIND_TONE, 1);
				} else {
                    tone_stop(0);
				}
			} else {
		...
		} else if(reason == SYNC_CMD_FIND_R_TONE){
            printf("SYNC_CMD_FIND_R_TONE\n");
			if (get_bt_tws_connect_status()){
...
static void play_tone_at_same_time(int tone_name, int err)
{
    ...
    case SYNC_TONE_TWS_CONNECTED:
        ...
        break;
#endif
        //sys_timeout_add((void*)SYNC_TONE_TWS_CONNECTED, bt_tws_tone_play, 1500);
        // tone_play_index(p_tone->tws_connect_ok, 1);
        break;
    case SYNC_TONE_PHONE_CONNECTED:
        if (err != -TWS_SYNC_CALL_RX) {
             tone_play_index(p_tone->bt_connect_ok, 1);
           // sys_timeout_add((void*)SYNC_TONE_PHONE_CONNECTED, bt_tws_tone_play, tone_get_status() ? 3000 : 300);
        }
        break;
