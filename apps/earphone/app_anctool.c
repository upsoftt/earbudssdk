...
static void app_anctool_ack_anc_mode(u8 cmd)
{
    ...
    } else if (cmd == CMD_ANC_BYPASS) {
        anc_mode_switch(ANC_BYPASS, 0);
#if ANC_HEARAID_EN
    } else if (cmd == CMD_ANC_HEARING_ON) {
        anc_hearaid_mode_switch(ANC_TRANSPARENCY, 1);
    } else if (cmd == CMD_ANC_HEARING_OFF) {
        anc_hearaid_mode_switch(ANC_OFF, 1);
...
static void anctool_ack_read_file_start(u32 id)
{
    ...
    case FILE_ID_GAIN:
        ...
        __this->file_hdl = (u8 *)malloc(__this->file_len);//数据指针
        if (__this->file_hdl) {
...
static void app_anctool_module_deal(u8 *data, u16 len)
{
    ...
    case CMD_ANC_PASS:
    case CMD_ANC_BYPASS:
    case CMD_ANC_HEARING_ON:
    case CMD_ANC_HEARING_OFF:
