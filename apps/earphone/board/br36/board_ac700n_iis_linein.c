...
#ifdef CONFIG_BOARD_AC700N_IIS_LINEIN
...
#if TCFG_APP_LINEIN_EN
    {KEY_ANC_SWITCH,   KEY_POWEROFF,  KEY_POWEROFF_HOLD,  KEY_NULL,     KEY_MODE_SWITCH,     KEY_NULL},   //KEY_0
    {KEY_VOL_DOWN, KEY_NULL,    KEY_NULL,         KEY_NULL,     KEY_NULL,        KEY_NULL},   //KEY_VOL-
    {KEY_VOL_UP, KEY_NULL,  KEY_NULL,       KEY_NULL,     KEY_NULL,      KEY_NULL},   //KEY_VOL+
#else
...
#if (IIS_ROLE == ROLE_M && TCFG_ADC_IIS_ENABLE && TCFG_APP_LINEIN_EN)
...
#if (TCFG_APP_LINEIN_EN && TCFG_ADC_IIS_ENABLE && IIS_ROLE == ROLE_M)
static void linein_det(void)
{
	u32 linein_det_voltage = 0;
	linein_det_voltage = adc_get_voltage(LINEIN_DET_CH);
	/* printf(">>>>>>>>>>>> linein det volatage = %d\n", linein_det_voltage); */
	if(linein_det_voltage <= 500 && app_get_curr_task() != APP_LINEIN_TASK){
		//linein引脚被插入, 当前是BT模式，则切换到linein模式
		printf(">>>>>>>>>>>>>>>>>>>>>>>> det linein wire insert!\n");
		if(app_get_curr_task() == APP_BT_TASK){
			extern int bt_app_exit(void *priv);
			bt_app_exit(NULL);
			//防止在切换到linein模式时手机回连播放连接成功的提示音
			if(app_var.wait_timer_do){
				sys_timeout_del(app_var.wait_timer_do);
				app_var.wait_timer_do = 0;
			}
		}
		app_task_switch_next();
	} else if(linein_det_voltage >= 2000 && app_get_curr_task() == APP_LINEIN_TASK) {
		//linein引脚没插入，但当前处于LINEIN模式，那么切换到下一个模式
		printf("Error: need insert linein wire!\n");
		extern void exit_linein_task(void);
		exit_linein_task();
		app_task_switch_next();
	}
}
...
{
	adc_add_sample_ch(LINEIN_DET_CH);
	gpio_set_die(LINEIN_DET_PORT, 0);
	gpio_set_direction(LINEIN_DET_PORT, 1);
	gpio_set_pull_up(LINEIN_DET_PORT, 1);
	gpio_set_pull_down(LINEIN_DET_PORT, 0);
	sys_timer_add(NULL, linein_det, 1000);
}
#endif
...
#if (TCFG_APP_LINEIN_EN && TCFG_ADC_IIS_ENABLE && IIS_ROLE == ROLE_M)
	//主机添加linein检测脚
	printf("************************************* master chip add linein det port\n");
	linein_det_init();
#endif
}
...
{
	//从机进入软关机之前，先设置为输入上拉，然后在使能唤醒功能, 否则会产生弱短路
#if (IIS_ROLE == ROLE_S && TCFG_ADC_IIS_ENABLE && TCFG_APP_LINEIN_EN)
	gSensor_wkupup_enable();
	gpio_set_direction(IIS_SLAVE_AWAKEN_PORT, 1);
    gpio_set_pull_up(IIS_SLAVE_AWAKEN_PORT, 1);
#elif (IIS_ROLE == ROLE_M && TCFG_ADC_IIS_ENABLE && TCFG_APP_LINEIN_EN)
	gSensor_wkupup_enable();
...
#elif (IIS_ROLE == ROLE_M && TCFG_ADC_IIS_ENABLE && TCFG_APP_LINEIN_EN)
	port_protect(port_group, IO_PORTB_01);	//iis主机唤醒io口保护
#else
