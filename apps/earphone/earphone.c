...
u8 need_default_volume = 80;// ?为初始化60%
...
static u16 tws_detach_shutdown_time = 0;
    ...
    if (!bt_audio_is_running()) {
        ...
        if (state & TWS_STA_PHONE_DISCONNECTED) {
			...
			if(tws_detach_shutdown_time == 0){
				tws_detach_shutdown_time = sys_timeout_add(NULL,power_set_soft_poweroff,8000);
			}
                sys_timer_del(app_var.wait_exit_timer);
                task_switch("idle", ACTION_IDLE_MAIN);
            }
...
{
#if TCFG_USER_TWS_ENABLE
    log_info("sylon debug :tws_api_get_role() = %d ",tws_api_get_role());
    if (tws_api_get_role() == TWS_ROLE_MASTER) {
        if (tws_api_get_tws_state() & TWS_STA_SIBLING_CONNECTED) {
            if (enable) {
...
void bt_sco_ctrl(unsigned char conn_sco){
    if(conn_sco){
...
static int bt_connction_status_event_handler(struct bt_event *bt)
{
    ...
    case BT_STATUS_INIT_OK:
        ...
        log_info("BT_STATUS_INIT_OK\n");
			...
			set_bt_version(BLUETOOTH_CORE_SPEC_60);  
...
static int event_handler(struct application *app, struct sys_event *event)
{
    ...
    log_info("sylon debug test >>>>>>>>>>>>> : event->type = %d  ",event->type);
    switch (event->type) {
    case SYS_KEY_EVENT:
        /*
         * 普通按键消息处理
...
static int state_machine(struct application *app, enum app_state state, struct intent *it)
{
    ...
    case APP_STA_CREATE:
        ...
        if (app_var.play_poweron_tone) {
            // STATUS *p_tone = get_tone_config();
            // int ret = tone_play_index(p_tone->power_on, 1);
