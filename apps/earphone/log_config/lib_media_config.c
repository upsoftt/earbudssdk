...
const int lib_config_aac_energy_det = TCFG_BT_SUPPORT_AAC;

/*PCM解码器解码帧长配置，0表示用默认(256points)帧长*/
const u16 lib_config_pcm_dec_frame_size = 0;
...
#endif/* EQ_LITE_CODE */
                               | EQ_FILTER_COEFF_FADE_EN//eq参数更新时使用淡入淡出
...
const int config_audio_dac_delay_off_ms = 1000;
#else
