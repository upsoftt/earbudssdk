...
uint8_t tuya_ble_cb_event_send(tuya_ble_cb_evt_param_t *evt)
{
    ...
    if (m_cb_queue_table[0]) {
        if (tuya_ble_os_msg_queue_send((void *)m_cb_queue_table[0], &event, 0)) {
            return 0;
        } else {
            return 1;
        }
...
void tuya_ble_adv_change(void)
{
    ...
    printf("adv data changed ,current bound flag = %d", tuya_ble_current_para.sys_settings.bound_flag);
