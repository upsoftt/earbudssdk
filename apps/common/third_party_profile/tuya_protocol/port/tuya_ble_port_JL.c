...
void tuya_get_vm_id_register(void (*handler)(u32 addr))
{
...
{
    if (ble_queue.enable) {
        if (p_handle == m_cb_queue_table[0]) {
            int argv[3];
            argv[0] = tuya_app_cb_handler;
            argv[1] = 1;
            argv[2] = p_msg;
...
uint8_t ble_characteristic_value_len;

uint8_t *tuya_get_ble_characteristic_value(uint8_t *len)
{
