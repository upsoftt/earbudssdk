...
u16 get_adv_setting_event_flag(void)
{
    return adv_setting_event_flag;
}

u8 deal_adv_setting_string_item(u8 *des, u8 *src, u8 src_len, u8 type)
{
    des[0] = type;
...
void update_adv_setting(u16 type)
{
    ...
    //                		total_len  time      name       key       led       mic      work       eq         vol      anc      anc_voice
    u16 adv_setting_to_sync_len = 1 + (4 + 1) + (32 + 1) + (8 + 1) + (3 + 1) + (1 + 1) + (1 + 1) + (11 + 1) + (8 + 1) + (25 + 1) + (4 + 1);
#if ATTR_TYPE_ASSISTED_HEARING
    adv_setting_to_sync_len += 3 + 2 * 4 * DHA_FITTING_CHANNEL_MAX + 1;
#endif
    u8 *adv_setting_to_sync = zalloc(adv_setting_to_sync_len);
    if (NULL == adv_setting_to_sync) {
        printf("%s alloc fail\n", __func__);
        return;
    }
    ...
    if (offset > 1) {
        ...
        if (type == ((u16) - 1)) {
            tws_api_send_data_to_sibling(adv_setting_to_sync, adv_setting_to_sync_len, TWS_FUNC_ID_ADV_SETTING_SYNC);
#if RCSP_ADV_LED_SET_ENABLE
    ...
    if (adv_setting_to_sync) {
        free(adv_setting_to_sync);
    }
}
...
#if RCSP_ADV_KEY_SET_ENABLE
    u8 key_setting_info[8] = {0};
    memset(key_setting_info, 0xff, 8);
