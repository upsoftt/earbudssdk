...
#if (TCFG_DEV_MANAGER_ENABLE && RCSP_ADV_EN)
...
#define ADV_APP_FILE_BROWSE_BUF_LEN             250//要小于RCSP MTU, 不要随意改大
#define ADV_APP_FILE_BROWSE_NAME_MAX_LIMIT      128//限制最大文件夹名称大小

#define ADV_APP_FILE_BROWSE_TASK_NAME           "file_bs"
#define ADV_APP_MAX_DEEPTH                      9/* 0~9 deepth of system */
...
enum {
    ADV_APP_BS_UNICODE = 0,
    ADV_APP_BS_ANSI,
};

static const char *dev_logo[] = {
    [ADV_APP_BS_UDISK] = "udisk0",
    [ADV_APP_BS_SD0] = "sd0",
    [ADV_APP_BS_SD1] = "sd1",
    [ADV_APP_BS_FLASH] = "virfat_flash",
    [ADV_APP_BS_FLASH_2] = "fat_nor",
};

#pragma pack(1)
struct __adv_browser {
    u8 path_type;///
    u8 read_file_num;
    u16 start_num;
    u32 dev_handle;
    u16 path_len;
    u32 path_clust[ADV_APP_MAX_DEEPTH];
};

struct JL_ADV_FILE_DATA {
    u8 type : 1;
    u8 format : 1;
    u8 device : 3;
    u8 reserve : 3;
...
static struct __adv_browser *browser = NULL;
...
static bool browser_get_dir_info(FILE_BS_DEAL *fil_bs, u8 *path_buf, u16 len, u8 type, void *ptr)
{
    ...
    if (len == 4 && type == ADV_APP_BS_FLODER) {
        /* printf("root deep\n"); */
        if (ptr) {
            *((u32 *)ptr) = ret;
        }
        goto end;
    } else if (len == 4 && type == ADV_APP_BS_FILE) {
...
{
    u32 file_cnt = 0;
    browser_get_dir_info(fil_bs, path_buf, len, ADV_APP_BS_FLODER, (void *)&file_cnt);
    return file_cnt;
}
...
{
    struct JL_ADV_FILE_DATA file_data;
    memset((u8 *)&file_data, 0, sizeof(struct JL_ADV_FILE_DATA));


    file_data.type = p_dir_info->dir_type;

    if (p_dir_info->fn_type == BS_FNAME_TYPE_SHORT) {
        file_data.format = ADV_APP_BS_ANSI;
    } else {
        file_data.format = ADV_APP_BS_UNICODE;
    }

    ///限制文件名长度
    p_dir_info->lfn_buf.lfn_cnt = adv_app_file_name_cut((u8 *)p_dir_info->lfn_buf.lfn, p_dir_info->lfn_buf.lfn_cnt, ADV_APP_FILE_BROWSE_NAME_MAX_LIMIT);
    ...
    memcpy(dest + offset, (u8 *)&file_data, sizeof(struct JL_ADV_FILE_DATA));
    memcpy(dest + offset + sizeof(struct JL_ADV_FILE_DATA), (u8 *)p_dir_info->lfn_buf.lfn, p_dir_info->lfn_buf.lfn_cnt);

    /* printf("add send data:"); */
    /* put_buf(dest+offset,p_dir_info->lfn_buf.lfn_cnt + sizeof(struct JL_ADV_FILE_DATA)); */
    return (p_dir_info->lfn_buf.lfn_cnt + sizeof(struct JL_ADV_FILE_DATA));
}

static void adv_app_browser_task(void *p)
{
    u8 reason = 0;
    ...
    fil_bs.dev = dev_manager_find_spec(adv_app_browser_dev_remap(browser->dev_handle), 0);
    if (fil_bs.dev == NULL) {
        reason = 1;
        printf("dev nofound!!!\n");
        goto _EXIT;
    }
    file_bs_open_handle(&fil_bs, (u8 *)adv_app_browser_file_ext());
    ...
    u16 offset = 0;
    u32 ret = 0;
    path_data = (u8 *)zalloc(ADV_APP_FILE_BROWSE_BUF_LEN);
    if (path_data == NULL) {
        reason = 1;
    ...
    for (int i = browser->start_num ; i < (browser->start_num + browser->read_file_num); i++) {
        ...
        if (offset && ((offset + dir_info.lfn_buf.lfn_cnt + sizeof(struct JL_ADV_FILE_DATA)) > ADV_APP_FILE_BROWSE_BUF_LEN)) {
            ...
            //reset send buf
            offset = 0;
            memset(path_data, 0, ADV_APP_FILE_BROWSE_BUF_LEN);
            offset += add_one_iterm_to_sendbuf(path_data, ADV_APP_FILE_BROWSE_BUF_LEN, offset, &dir_info, i, browser->dev_handle);
        } else {
            offset += add_one_iterm_to_sendbuf(path_data, ADV_APP_FILE_BROWSE_BUF_LEN, offset, &dir_info, i, browser->dev_handle);
        }
    }
...
_EXIT:
    ...
    u8 bs_param[6] = {0};
    bs_param[0] = reason;
    bs_param[1] = (u8) browser->dev_handle;
    memcpy(bs_param + 2, play_file_clust, sizeof(play_file_clust));
    JL_rcsp_event_to_user(DEVICE_EVENT_FROM_RCSP, MSG_JL_FILE_BROWSER_END, bs_param, sizeof(bs_param));
...
{
    ///检查数据是否有效
    if (len > sizeof(struct __adv_browser)) {
        return ;
    }
    ///创建数据解析句柄
    browser = (struct __adv_browser *)zalloc(sizeof(struct __adv_browser));
    if (browser == NULL) {
        return ;
    }
    ///解析数据
    memcpy((u8 *)browser, data, sizeof(struct __adv_browser));
    ...
    ///检查设备范围
    if (browser->dev_handle >= ADV_APP_BS_DEV_MAX) {
    ...
    ///目录浏览线程创建
    if (task_create(adv_app_browser_task, (void *)NULL, ADV_APP_FILE_BROWSE_TASK_NAME)) {
        free(browser);
        browser = NULL;
...
{
    ///删除线程，释放资源
    task_kill(ADV_APP_FILE_BROWSE_TASK_NAME);
    if (browser) {
#if 0
        struct __dev *dev;
        dev = dev_manager_find_spec(adv_app_browser_dev_remap(browser->dev_handle), 0);
        if (dev != NULL) {
...
static void adv_app_file_browser_stop_resp(u8 reason)
{
...
{
    int argv[3];
    argv[0] = (int)adv_app_file_browser_stop_resp;
    argv[1] = 1;
    argv[2] = reason;
