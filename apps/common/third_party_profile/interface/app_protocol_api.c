...
static int app_protocol_necm_init(int match_id);
static int app_protocol_wsp_init(int match_id);
...
struct app_ctrl_operation_t {
    ...
    int(*update_bt_state_to_lib)(int sta);
};
...
void app_protocal_update_bt_state_to_lib(int state)
{
    app_protocol_interface_t *app;
    list_for_each_app_protocol(app) {
        if (app->app_ctrl) {
            if (app->app_ctrl->update_bt_state_to_lib) {
                app->app_ctrl->update_bt_state_to_lib(state);
            }
        }
    }
}
...
static int app_protocol_message_handler(int id, int opcode, u8 *data, u32 len)
{
    ...
    APP_PROTOCOL_LOG("\napp_protocol_message_handler opcode = 0x%x\n", opcode);
    int ret = 0;
    switch (opcode) {
...
#endif
#if APP_PROTOCOL_NECM_CODE
        if (app_protocol_necm_init(handler_id)) {
            __this->running_protocol_number++;
        }
#endif
#if APP_PROTOCOL_WSP_CODE
        if (app_protocol_wsp_init(handler_id)) {
            __this->running_protocol_number++;
        }
#endif
    }
...
void app_protocol_all_init()
{
    ...
    app_protocol_init(NECM_HANDLER_ID);
}
...
extern struct app_protocol_private_handle_t dma_private_handle;
...
static int app_protocol_dma_init(int match_id)
{
    ...
    if (app) {
        ...
        if (app) {
            ...
            app_protocol_handle_register(&dma_private_handle);
...
{
    printf("realme_product_test_recieve_cbk, len:", len);
    put_buf(buf, len);
...
extern struct app_protocol_private_handle_t necm_private_handle;
AI_RECORD_REGISTER(necm_record_item) = {
    .service_record = (u8 *)sdp_spp_service_data,
    .service_record_handle = 0x00010004,
};
static const struct app_ctrl_operation_t cloudmusic_ctrl = {
    .protocol_init = cloudmusic_all_init,
    .protocol_exit = cloudmusic_all_exit,
};
static int app_protocol_necm_init(int match_id)
{
    /* 变量用于控制单击是否支持取消收藏功能，0表示不支持，1表示支持 */
    u8 cloudmusic_support_unstar = 0;
    if (DEMO_HANDLER_ID != match_id) {
        return 0;
    }
    APP_PROTOCOL_LOG("app_protocol_demo_init\n");
    app_protocol_interface_t *app = get_interface_for_id(NECM_HANDLER_ID);
    if (app) {
        APP_PROTOCOL_LOG("DEMO_HANDLER_ID init again\n");
    } else {
        app = get_empty_interface();
        if (app) {
            app->handler_id = NECM_HANDLER_ID;
            app->loop = NULL;
            app->settings = NULL;
            app->app_ctrl = &cloudmusic_ctrl;
            app->callback = NULL;
            app_protocol_common_init(app);
            APP_PROTOCOL_LOG("DEMO_HANDLER_ID init OK\n");
            return 1;
...
#if APP_PROTOCOL_WSP_CODE
static const struct app_ctrl_operation_t wsp_ctrl = {
    .protocol_init = wsp_all_init,
    .protocol_exit = wsp_all_exit,
    .update_bt_state_to_lib = wsp_update_bt_state_to_lib,
};

//wsp连接方式:
//0x00:单ble, 0x01:单edr, 0x02:edr+ble,需要同地址
static u8 wsp_conn_type = 0x01;

static u8 wsp_class[3] = {0x20, 0x04, 0x04};
extern struct app_protocol_private_handle_t wsp_private_handle;
static int app_protocol_wsp_init(int match_id)
{
    if (DEMO_HANDLER_ID != match_id) {
        return 0;
    ...
    APP_PROTOCOL_LOG("app_protocol_demo_init\n");
    app_protocol_interface_t *app = get_interface_for_id(WSP_HANDLER_ID);
    if (app) {
        APP_PROTOCOL_LOG("DEMO_HANDLER_ID init again\n");
    } else {
        app = get_empty_interface();
        if (app) {
            app->handler_id = WSP_HANDLER_ID;
            app->loop = NULL;
            app->settings = NULL;
            app->app_ctrl = &wsp_ctrl;
            app->callback = NULL;
            // 定时单位为毫秒，传0表示不设置配对模式超时
            wsp_pair_timer_set(0);
            wsp_beacon_conn_type_set(wsp_conn_type);
            wsp_class_of_device_set(wsp_class);
            app_protocol_common_init(app);
            APP_PROTOCOL_LOG("WSP_HANDLER_ID init OK\n");
            return 1;
        } else {
            APP_PROTOCOL_LOG("not support more app protocal\n");
        }
