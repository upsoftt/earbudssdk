...
///Type 1 Format type descriptor
    ...
    SPK_AUDIO_RES_16,      //BitsResolution:16bit
    0x03,      //SampleFreqType:One sampling frequency.
    DW1BYTE(SPK_AUDIO_RATE_44_1K), DW2BYTE(SPK_AUDIO_RATE_44_1K), DW3BYTE(SPK_AUDIO_RATE_44_1K),
    DW1BYTE(SPK_AUDIO_RATE_48K), DW2BYTE(SPK_AUDIO_RATE_48K), DW3BYTE(SPK_AUDIO_RATE_48K),
    DW1BYTE(SPK_AUDIO_RATE_96K), DW2BYTE(SPK_AUDIO_RATE_96K), DW3BYTE(SPK_AUDIO_RATE_96K),
...
//Isochronous,Synchronization Type(Asynchronous)
    ...
    USB_ENDPOINT_XFER_ISOC | USB_ENDPOINT_SYNC_SYNC, //0x09,
...
///Type 1 Format type descriptor
    ...
    SPK_AUDIO_RES_24,      //BitsResolution:16bit
    0x03,      //SampleFreqType:One sampling frequency.
    DW1BYTE(SPK_AUDIO_RATE_44_1K), DW2BYTE(SPK_AUDIO_RATE_44_1K), DW3BYTE(SPK_AUDIO_RATE_44_1K),
    DW1BYTE(SPK_AUDIO_RATE_48K), DW2BYTE(SPK_AUDIO_RATE_48K), DW3BYTE(SPK_AUDIO_RATE_48K),
    DW1BYTE(SPK_AUDIO_RATE_96K), DW2BYTE(SPK_AUDIO_RATE_96K), DW3BYTE(SPK_AUDIO_RATE_96K),
...
static u32 spk_pc_samplerate = 48000;   //PC端设定的采用率
static u32 spk_pc_resolution = 16;      //PC端设定的位宽
static u32 uac_endpoint_recv(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case SPK_ISO_EP_OUT|USB_DIR_OUT:
        ...
        memcpy(&spk_pc_samplerate, read_ep, 3);
        printf("recv sample spk!!!  %d\n", spk_pc_samplerate);
...
static void spk_transfer(struct usb_device_t *usb_device, u32 ep)
{
    ...
    u32 spk_frame_len = spk_pc_samplerate * (spk_pc_resolution / 8) * SPK_CHANNEL / 1000;
    spk_frame_len += (spk_pc_samplerate % 1000 ? (spk_pc_resolution / 8) * SPK_CHANNEL : 0);
...
static void open_spk(struct usb_device_t *usb_device)
{
    ...
    spk_frame_len += (spk_pc_samplerate % 1000 ? (spk_pc_resolution / 8) * SPK_CHANNEL : 0);
    u8 *ep_buffer = uac_info->spk_dma_buffer;
    uac_speaker_stream_open(spk_pc_samplerate, SPK_CHANNEL, spk_pc_resolution);
    ...
    case USB_REQ_SET_INTERFACE:
        if (usb_device->bDeviceStates == USB_DEFAULT) {
            ...
            if (SetInterface_0_Lock == 0) {
                ...
                usb_set_setup_phase(usb_device, USB_EP0_STAGE_SETUP);
                if (setup->wValue == 2) { //alt 2
                    spk_pc_resolution = SPK_AUDIO_RES_24;
                    open_spk(usb_device);
                } else if (setup->wValue == 1) { //alt 1
                    spk_pc_resolution = SPK_AUDIO_RES_16;
                    open_spk(usb_device);
...
u32 uac_audio_desc_config(const usb_dev usb_id, u8 *ptr, u32 *cur_itf_num)
{
    ...
    tptr[2] = *cur_itf_num + 1;
    tptr[9 + 2] = *cur_itf_num + 1;
    frame_len = SPK_AUDIO_RATE_96K * (SPK_AUDIO_RES_16 / 8) * SPK_CHANNEL / 1000;
    frame_len += (SPK_AUDIO_RATE_96K % 1000 ? (SPK_AUDIO_RES_16 / 8) * SPK_CHANNEL : 0);
    offset = 9 + 9 + 7 + 17 + 4;
    //MaxPacketSize of spk iso out
    tptr[offset] = LOBYTE(frame_len);
    tptr[offset + 1] = HIBYTE(frame_len);

    tptr[58 + 2] = *cur_itf_num + 1;
    tptr[58 + 9 + 2] = *cur_itf_num + 1;
    frame_len = SPK_AUDIO_RATE_96K * (SPK_AUDIO_RES_24 / 8) * SPK_CHANNEL / 1000;
    frame_len += (SPK_AUDIO_RATE_96K % 1000 ? (SPK_AUDIO_RES_24 / 8) * SPK_CHANNEL : 0);
    offset = 58 + 9 + 7 + 17 + 4;
    //MaxPacketSize of spk iso out
    tptr[offset] = LOBYTE(frame_len);
    tptr[offset + 1] = HIBYTE(frame_len);
