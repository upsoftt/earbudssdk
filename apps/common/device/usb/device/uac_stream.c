...
static void uac_streamon_handler(struct uac_speaker_handle *uac);
void uac_speaker_stream_write(const u8 *obuf, u32 len)
{
    if (speaker_stream_is_open) {
        ...
        /* #endif */
        uac_speaker->streamon = 1;
        uac_streamon_handler(uac_speaker);
...
static void uac_streamon_handler(struct uac_speaker_handle *uac)
{
    ...
    if (uac->pp_event == 1) {
        ...
        event.u.dev.value = (int)((uac->channel << 28) | (uac->bit_wide << 20) | uac->sample_rate);
...
static void uac_stream_detect_timer(void *arg)
{
    ...
    local_irq_disable();
    if (!uac_speaker->streamon) {
        if (uac_speaker->pp_event == 0) {
            uac_speaker->pp_event = 1;
    ...
    uac_streamon_handler(uac_speaker);
    uac_speaker->streamon = 0;
    local_irq_enable();
}

void uac_speaker_stream_open(u32 samplerate, u32 ch, u8 spk_pc_resolution)
{
    if (speaker_stream_is_open) {
        return;
    }
    ...
    if (!uac_speaker) {
        ...
        uac_speaker->sample_rate = samplerate;
        uac_speaker->bit_wide = (spk_pc_resolution == 24) ? BIT_WIDE_24BIT : BIT_WIDE_16BIT;
        /* #ifdef CONFIG_MEDIA_DEVELOP_ENABLE */
...
#if 1
    if (!uac_speaker->stream_detect_timer) {
        uac_speaker->streamon = 0;
        uac_speaker->pp_event = 1;
        uac_speaker->stream_detect_timer = sys_hi_timer_add(NULL, uac_stream_detect_timer, 5);
    }
    ...
    event.arg = (void *)DEVICE_EVENT_FROM_UAC;
    event.u.dev.event = USB_AUDIO_PLAY_OPEN;
    event.u.dev.value = (int)((ch << 28) | (uac_speaker->bit_wide << 20) | samplerate);

#if !UAC_DEBUG_ECHO_MODE
    sys_event_notify(&event);
#endif /*UAC_DEBUG_ECHO_MODE*/
...
void uac_speaker_stream_close()
{
    ...
    if (uac_speaker) {
        ...
        if (uac_speaker->stream_detect_timer) {
            sys_hi_timer_del(uac_speaker->stream_detect_timer);
        }
