...
typedef enum _syscfg_err {
//SYSCFG
    SYSCFG_MALLOC_ERROR = -230,              //写缓冲区申请失败
} SYSCFG_ERR;
...
struct syscfg_ex_operations {
    int (*force_write)(u16 item_id, u8 *buf, u16 len);
};
...
#define     VM_CHARGE_CALIBRATION       108//用于存储充电校准值
