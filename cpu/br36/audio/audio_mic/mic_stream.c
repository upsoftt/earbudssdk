...
struct __mic_stream *mic_stream_creat(struct __mic_stream_parm *parm)
{
    ...
    stream->parm = p;
    stream->ladc_buf = zalloc(p->point_unit * 2 * 2);
    os_sem_create(&stream->sem, 0);
    err  = os_task_create(mic_stream_task_deal, (void *)stream, 5, 768, 128, MIC_STREAM_TASK_NAME);
...
/* #define LADC_CH_NUM        	1	#<{(|支持的最大采样通道(max = 2)|)}># */
/* #define LADC_BUF_NUM        2	#<{(|采样buf数|)}># */
/* #define LADC_IRQ_POINTS     256	#<{(|采样中断点数|)}># */
/* #define  LADC_DEMO_BUFS_SIZE      (LADC_CH_NUM * LADC_BUF_NUM * LADC_IRQ_POINTS) */

/* s16 ladc_buf[LADC_DEMO_BUFS_SIZE]; */

bool mic_stream_start(struct __mic_stream  *stream)
{
    if (stream) {
#if 1
        audio_adc_mic_open(&stream->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
        audio_adc_mic_set_gain(&stream->mic_ch, stream->parm->mic_gain);


        audio_adc_mic_set_sample_rate(&stream->mic_ch, stream->parm->sample_rate);
        audio_adc_mic_set_buffs(&stream->mic_ch, stream->ladc_buf, stream->parm->point_unit * 2, 2);
        stream->adc_output.handler = adc_output_to_buf;
        stream->adc_output.priv = stream;
        audio_adc_add_output_handler(&adc_hdl, &stream->adc_output);
        //step4:启动mic通道采样
        audio_adc_mic_start(&stream->mic_ch);
...
void mic_stream_destroy(struct __mic_stream **hdl)
{
    ...
    audio_adc_del_output_handler(&adc_hdl, &stream->adc_output);
    audio_adc_mic_close(&stream->mic_ch);
