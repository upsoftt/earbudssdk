...
static int uac_stream_read(struct audio_decoder *decoder, void *buf, u32 len)
{
    ...
    if (dec->bit_wide == BIT_WIDE_24BIT) {
        len = len * 3 / 2;
        if (!dec->buf) {
            dec->buf = zalloc(len);
        }
        rlen = uac_speaker_read(NULL, (void *)dec->buf, len);
        threebyte_24bit_to_16bit((int *)dec->buf, (int *)buf, rlen / 3);
        rlen = rlen * 2 / 3;
    } else {
...
static int usb_audio_play_open(void *_info)
{
    ...
    dec->sample_rate = (u32)_info & 0xFFFFF;
    dec->bit_wide = ((u32)_info >> 20) & 0xFF;
    dec->channel = (u32)_info >> 28;
    printf("usb_audio_play_open sr:%d ch:%d bit_wide:%d\n", dec->sample_rate, dec->channel, dec->bit_wide); //通道是2个
...
int uac_dec_restart(int magic)
{
    ...
    int _info = (uac_dec->channel << 28) | (uac_dec->bit_wide << 20) | uac_dec->sample_rate;
