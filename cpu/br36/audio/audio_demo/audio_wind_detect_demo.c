...
/*mic初始化*/
static int audio_mic_en(u8 en, u16 sr, u16 mic0_gain, u16 mic1_gain)
{
    ...
    if (en) {
        ...
        if (en & LADC_CH_MIC_L) {
            ...
            audio_adc_mic_open(&audio_mic->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
            audio_adc_mic_set_gain(&audio_mic->mic_ch, mic0_gain);
        }
        if (en & LADC_CH_MIC_R) {
            printf("adc_mic1 open,gain:%d\n", mic1_gain);
            audio_adc_mic1_open(&audio_mic->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
...
/*打开风噪检测*/
int audio_wind_detect_open(u32 sr, u16 mic_gain)
{
    int err = 0;
    audio_cvp_code_load();
    audio_aec_open(sr, WNC_EN, audio_wind_detect_output_hdl);
    audio_mic_en(TCFG_AUDIO_ADC_MIC_CHA, sr, mic_gain, mic_gain);
    return 0;
}
...
{
    /*打开风噪检测*/
    audio_wind_detect_open(16000, 8);
    /*获取风噪的信息*/
