...
#if (TCFG_AD2DA_LOW_LATENCY_MIC_CHANNEL & LADC_CH_MIC_L)
        audio_adc_mic_open(&hdl->mic_ch, param->mic_ch_sel, &adc_hdl);
        audio_adc_mic_set_gain(&hdl->mic_ch, param->mic_gain);
#endif/*TCFG_AD2DA_LOW_LATENCY_MIC_CHANNEL & LADC_CH_MIC_L*/
#if (TCFG_AD2DA_LOW_LATENCY_MIC_CHANNEL & LADC_CH_MIC_R)
        audio_adc_mic1_open(&hdl->mic_ch, param->mic_ch_sel, &adc_hdl);
        audio_adc_mic1_set_gain(&hdl->mic_ch, param->mic_gain);
#endif/*(TCFG_AD2DA_LOW_LATENCY_MIC_CHANNEL & LADC_CH_MIC_R)*/
        audio_adc_mic_set_sample_rate(&hdl->mic_ch, param->sample_rate);
        audio_adc_mic_set_buffs(&hdl->mic_ch, hdl->adc_dma_buf, param->adc_irq_points * 2, param->adc_buf_num);
...
#if (TCFG_EQ_ENABLE && DHA_EQ2_ENABLE)
    hdl->hearing_eq2 = hearing_eq2_open(param->sample_rate, 1);
