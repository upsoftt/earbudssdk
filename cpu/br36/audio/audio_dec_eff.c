...
void *dec_eq_drc_setup(void *priv, int (*eq_output_cb)(void *, void *, int), u32 sample_rate, u8 channel, u8 async, u8 drc_en)
{
    ...
    async = 0;
#endif
    if (async) { //异步时eq接在 syncts后，需使用与syncts一样的采样率
        sample_rate = 	audio_mixer_get_sample_rate(&mixer) ? audio_mixer_get_sample_rate(&mixer) : sample_rate;
    }
...
int spk_eq_spp_rx_packet(u8 *packet, u8 len)
{
    ...
    case CMD_SUPPORT_GLOBAL_GAIN:
        ...
        break;
    case CMD_READ_SEG_L:
        app_online_db_ack(parse_seq, spk_eq_seg_tab, sizeof(spk_eq_seg_tab) / 2);
        return 1;
    case CMD_READ_SEG_R:
        app_online_db_ack(parse_seq, &spk_eq_seg_tab[10], sizeof(spk_eq_seg_tab) / 2);
        return 1;
    case CMD_READ_GLOBAL_L:
        app_online_db_ack(parse_seq, &spk_eq_global_gain[0], 4);
        return 1;
    case CMD_READ_GLOBAL_R:
        app_online_db_ack(parse_seq, &spk_eq_global_gain[1], 4);
        return 1;
