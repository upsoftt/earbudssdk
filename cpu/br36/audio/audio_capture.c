...
static void adc_dual_mic_output(void *priv, s16 *data, int len)
{
...
int audio_mic_init(u16 sr)
{
    ...
    AC_LOG("[AC]mic_gain:%d,%d\n", mic0_gain, mic1_gain);

    if (adc_dm) {
#if (ADC_DM_CH_NUM == 2)
        audio_adc_mic_open(&adc_dm->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);
#endif
        audio_adc_mic1_open(&adc_dm->mic_ch, AUDIO_ADC_MIC_CH, &adc_hdl);

        audio_adc_mic_set_sample_rate(&adc_dm->mic_ch, sr);
        audio_adc_mic_set_gain(&adc_dm->mic_ch, mic0_gain);
        audio_adc_mic1_set_gain(&adc_dm->mic_ch, mic1_gain);
        audio_adc_mic_set_buffs(&adc_dm->mic_ch, adc_dm->adc_buf, ADC_DM_IRQ_POINTS * 2, ADC_DM_BUF_NUM);
        adc_dm->adc_output.handler = adc_dual_mic_output;
