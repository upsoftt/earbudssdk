...
static void pwm_led_two_io_mode_slot_timer_display(u8 display)
{
    ...
    if (__this->user_data->io_mode == LED_THREE_IO_MODE) {
        led_pin_set_disable(__this->user_data->io_cfg.three_io.pin0);
        led_pin_set_disable(__this->user_data->io_cfg.three_io.pin1);
        led_pin_set_disable(__this->user_data->io_cfg.three_io.pin2);
    }
    ...
    if ((tws_api_get_tws_state() & TWS_STA_SIBLING_DISCONNECTED) || (led_module_is_in_sniff_mode())) {
        ...
        if (led_module_is_in_sniff_mode() && ((__this->last_mode == PWM_LED0_ONE_FLASH_5S) || (__this->last_mode == PWM_LED1_ONE_FLASH_5S))) {
            LED_PWM_DISABLE; //进sniff时在sniff节奏里面闪
        }
        return;
    }
...
//双灯互闪
    case PWM_LED0_LED1_FAST_FLASH: //使用中断切灯
        isr_mode = PWM_LED0_LED1_FAST_FLASH;
        break;
    case PWM_LED0_LED1_SLOW_FLASH:
        isr_mode = PWM_LED0_LED1_SLOW_FLASH;
        break;
    ...
    if (change_mode) {
        pwm_led_one_io_mode_slot_timer_display(change_mode);
        LED_PWM_DISABLE;
        LED_BRI_DUTY1_SYNC0();
        if (led2_enable) {
            led_pin_set_enable(__this->user_data->io_cfg.three_io.pin2);
...
#define __timer_16slot_reset(timer, slot_num) \
	do { \
		if (timer != 0xFF) { \
			timer_16slot_reset(timer, slot_num); \
		} \
	} while(0) \
...
static void pwm_led_one_io_mode_slot_timer_display(u8 display)
{
    ...
    if ((tws_api_get_tws_state() & TWS_STA_SIBLING_DISCONNECTED) || (led_module_is_in_sniff_mode())) {
        //如果tws未连接, 使用lrc节奏
        if (__this->user_data->io_mode == LED_TWO_IO_MODE || __this->user_data->io_mode == LED_THREE_IO_MODE) {
            //printf("led two io mode: %d", display);
            pwm_led_two_io_mode_display(display);
        } else {
            pwm_led_one_io_mode_display(display);
        }
...
//双灯互闪
    ...
    if (need_slot_timer) {
        if (led_index == 2) {
            ...
            led_slot_timer.double_cnt = 0;
            led_slot_timer.light_slot_num = __pwm_led_get_16slot_num(light_time + 50);
            _pwm_led_one_flash_display(led_index, CFG_LED0_LIGHT, CFG_LED1_LIGHT, 7000, 5, light_time);
            LED_PWM_DISABLE; //默认从灭灯开始
            pwm_led_16slot_timer_set(period - light_time);
        }
    }
